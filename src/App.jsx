import { startOAuthFlow, logout } from './auth'
            ...
            <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px' }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24, flexWrap:'wrap', gap:12 }}>
                <div>
                  ...
                  <p style={{ color:'#7a8ba0', fontSize:13, marginTop:4 }}>Karols Portfolio · Seit 05.02.2023 · Nettowerte</p>
                </div>
              </div>
                  ...
                  <>
                    ...
                    <div style={{ display:'flex', gap:14, flexWrap:'wrap', marginBottom:24 }}>
                      ...
                      <KpiCard
                          ...
                          value={(() => {
                            const forecast2026 = (forecastMonthly?.[cy] || []).reduce((s, v) => s + v, 0)
                            const actual2025   = (Object.values(monthly[cy - 1] || [])).reduce((s, v) => s + v, 0)
                            if (actual2025 === 0) return '–'
                            const growth = ((forecast2026 - actual2025) / actual2025) * 100
                            return (growth >= 0 ? '+' : '') + growth.toFixed(1).replace('.', ',') + ' %'
                          })()}
                          color={(() => {
                            const forecast2026 = (forecastMonthly?.[cy] || []).reduce((s, v) => s + v, 0)
                            const actual2025   = (Object.values(monthly[cy - 1] || [])).reduce((s, v) => s + v, 0)
                            if (actual2025 === 0) return '#7a8ba0'
                            const growth = ((forecast2026 - actual2025) / actual2025) * 100
