...
export default function DividendChart({ monthly, cum, forecastCum, forecastMonthly, byHolding, forecastByHolding }) {
    ...
    const monthlyBarData = MONTHS.map((name, i) => {
        ...
        years.forEach(y => {
            if (y < cy) {
                pt[String(y)] = +(monthly[y]?.[i] || 0).toFixed(2)
            } else {
                pt[String(y)] = i < cm ? +(monthly[y]?.[i] || 0).toFixed(2) : null
            }
        })
        if (i >= cm) pt['prognose'] = +fmCy[i].toFixed(2)
        return pt
    })
    ...
    const cumLineData = MONTHS.map((name, i) => {
        ...
        years.filter(y => y < cy).forEach(y => {
            pt[String(y)] = +(cum[y]?.[i] || 0).toFixed(2)
        })
        pt[`${cy}_real`]     = i <= cm - 1 ? fcCy[i] : null
        pt[`${cy}_forecast`] = i >= cm - 1 ? fcCy[i] : null
        pt[`${ny}_forecast`] = fcNy[i] ?? null
        return pt
    })

    // Nach Aktie – gesamte Zeitspanne
    ...
    const allYears = isins.flatMap(isin =>
        Object.keys(byHolding[isin].monthly || {}).map(Number)
    )
    ...
    const stackedData = Array.from({ length: totalMonths }, (_, i) => {
        ...
        for (const isin of isins) {
            ...
            const val = isFuture
                ...
                : +(h.monthly[year]?.[month] || 0).toFixed(2)
            pt[isin] = val
        }
        return pt
    })
    ...
    const renderLineLegend = value => {
        ...
        const label = value === `${cy}_real`     ? String(cy)
                ...
                <div style={{ display:'flex', gap:8 }}>
                    {MODES.map(({ key, label }) => (
                        <button key={key} onClick={() => setMode(key)} style={{
                            ...
                            background: mode === key ? '#1e3a5f' : 'transparent',
                            color:      mode === key ? '#93c5fd' : '#7a8ba0',
                        }}>
                            {label}
                        </button>
                    ))}
                ...
                {mode === 'monthly' ? (
                    <BarChart data={monthlyBarData} margin={{ top:8, right:16, left:0, bottom:0 }} barCategoryGap="20%" barGap={2}>
                        ...
                        <Tooltip
                            ...
                            formatter={(v, n) => [`${(+v).toFixed(2)} €`, n === 'prognose' ? 'Prognose' : n]}
                            ...
                            content={({ active, payload, label }) => {
                                ...
                                return (
                                    <div style={{ background:'#1a2233', border:'1px solid #222d3d', borderRadius:8, padding:'10px 14px' }}>
                                        <p style={{ color:'#c8d4e0', marginBottom:6, fontWeight:600 }}>{label}</p>
                                        {filtered.map(p => (
                                            <p key={p.dataKey} style={{ color: p.fill, margin:'2px 0', fontSize:13 }}>
                                                {byHolding[p.dataKey]?.name || p.dataKey} : {(+p.value).toFixed(2)} €
                                            </p>
                                        ))}
                                    </div>
                                )
                    ...
                    <LineChart data={cumLineData} margin={{ top:8, right:16, left:0, bottom:0 }}>
                        ...
                        <Tooltip
                            ...
                            labelStyle={{ color:'#c8d4e0' }}
                            formatter={(v, n) => {
                                const label = n === `${cy}_real` ? `${cy}` : n.endsWith('_forecast') ? 'Prognose' : n
